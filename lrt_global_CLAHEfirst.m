function out_img = lrt_global_CLAHEfirst( img, source_Y, target_dr, max_Y )
...
%luma_in = adapthisteq(luma_in,'NumTiles',[32 32],'ClipLimit',0.02);

%% Start for PDP
luma_in_forTC = get_luminance( img, 0);
figure('Name','luma_in_forTC');imshow(luma_in_forTC);
luma_in_forTC_CLAHE = adapthisteq(luma_in_forTC,'NumTiles',[8 8],'ClipLimit',3/255);
figure('Name','luma_in_forTC after CLAHE');imshow(luma_in_forTC_CLAHE)
...
mean_luma_in = mean(luma_in_forTC_CLAHE, 'all');
...
luma_tc_newTC = interp1( T.tcs.pix_val, luma_out_lut, luma_in_forTC_CLAHE );
luma_tc_newTC = clamp(luma_tc_newTC, 0.00001, 1);
luma_in_forTC_CLAHE = clamp(luma_in_forTC, 0.00001, 1);
%luma_tc_newTC = adapthisteq(luma_tc_newTC,'NumTiles',[8 8],'ClipLimit',3/255);

ratio = luma_tc_newTC ./ luma_in_forTC_CLAHE;
...
%y_out = adapthisteq(y_out,'NumTiles',[8 8],'ClipLimit',0.02);
