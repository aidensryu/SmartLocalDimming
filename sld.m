function out_img = sld( img, source_Y, target_dr, amb, max_Y, c_nl, LocalY )
...
%LocalY = imbilatfilt(LocalY, 3);
% LocalY = double(imread('images/A09 SimulationBacklightDiffusionModule_local dimming_avg.png'))/255;
% LocalY = LocalY - min(min(LocalY));
% LocalY = LocalY / max(max(LocalY));
LocalY = LocalY / max(max(LocalY));
LocalY = power(LocalY, c_nl);

%LocalY = power(LocalY, 9);

%LocalY = imresize(LocalY, size(luma_in_forTC));
% thresh = multithresh(LocalY, 20);
% seg_I = imquantize(LocalY, thresh);
% RGB = label2rgb(seg_I); 	
% figure;imshow(imresize(RGB, 5));
% thresh2 = thresh;
% for i=1:19
%     thresh2(i) = (thresh(i)+thresh(i+1))/2;
% end
% thresh2(20) = max(max(LocalY));
% thresh = [(min(min(LocalY))+thresh(1))/2 thresh2];
% 
% imwrite(RGB, 'lrt_SLD_seg.png');
% for y=1:1920
%     for x=1:720
%         seg_I(x,y) = thresh(max(seg_I(x,y)));
%     end
% end
% seg_I = power(seg_I, 1);
% LocalY = seg_I;% * max(max(LocalY))/20; %% This part should be corrected --> not quantized label, 
 LocalY = imresize(LocalY, 0.125);
LocalY = LocalY * desiredY;
LocalY = max(minY*10.0, LocalY);

% min(min(LocalY))
% max(max(LocalY))
% mean(mean(LocalY))
% 
imwrite(LocalY/max_Y, 'lrt_SLD_localY.png');
% figure('Name', 'LocalY');imshow(LocalY,[]);
%% TC computation
ratio_local = luma_in_forTC;
...
for y=1:n_y
    for x=1:n_x
        i = n_x*(y-1) + x; 
        dr_out = (log10(LocalY(i)/minY)-T.tcs.dr_out(1))/(T.tcs.dr_out(end)-T.tcs.dr_out(1))*(length(T.tcs.dr_out)-1);
        
        dr_out_index = floor(dr_out);
        dr_out_res = dr_out - dr_out_index;
        
%         if(dr_out_index < 0)
% %             print('waring! dr_out is out of range');
%             dr_out_index
%             LocalY(i)
%             LocalY(i)./minY
%             dr_out
%         end
        ...
        dr_out_index2 = min(15, max(dr_out_index+1, 0));

        % Discrete from dr_out_index
        %     if(dr_out_index)
        %         dr_out_index;
        %     end
        tc_mat1 = T.tcs.(sprintf('tc%d', dr_out_index));   
        tc_mat2 = T.tcs.(sprintf('tc%d', dr_out_index2));   
        tc_mat = tc_mat1 .* (1-dr_out_res) + tc_mat2 .* (dr_out_res);
...
%         imshow(imresize(luma_in_local, 10));
...
% figure('Name', 'ratio_local');imshow(ratio_local,[]);
% imwrite(ratio_local, 'ratio_local_sld.png');

%ratio_local = imguidedfilter(ratio_local, luma_in);
% ratio_local = imgaussfilt(ratio_local, 15);
