function [res, gaussian] = laplacian_pyramid_dec(img,levels,kernel_a)
...
%   laplacian_pyramid(img,levels) - kernel_a set to be equal to 0.4
    ...
    for i = 2:height
        g_pyramid{i} = imresize(g_pyramid{i}(1:2^(i-1):end, 1:2^(i-1):end),size(g_pyramid{i}), 'bilinear');
    end
    for i=1:height-1
        res{i} = g_pyramid{i} - g_pyramid{i+1};
    end
    res{height} = g_pyramid{height};%g_pyramid{height};
    if nargout > 1
       gaussian = g_pyramid; 
    end
